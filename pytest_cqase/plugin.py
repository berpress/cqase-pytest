...
def get_ids_from_pytest_nodes(items):
    ...
    return (
        {
            item.nodeid: {"ids": item.get_closest_marker(QASE_MARKER).kwargs.get("ids")}
            for item in items
...
class QasePytestPlugin:
    ...
    def __init__(
        ...
        self.client = QaseClient(api_token)
        ...
        self.existing_ids = []
        self.last_node = None
        self.project = self.client.projects.get_project_by_code(self.project_code)
        self.comment = "Pytest Plugin Automation Run"
        if not self.project:
            raise ValueError("Unable to find given project code")
        self.check_testrun()
        if self.debug:
            logger = logging.getLogger("cqase_client")
    ...
    def check_case_ids(self, data):
        ...
        for _id in data.get("ids"):
            case = self.client.cases.get_case_by_id(self.project_code, _id)
            if case:
    ...
    def check_testrun(self):
        if (self.testrun_id or self.create_run) and self.testplan_id:
            raise ValueError("You should provide either use testrun or testplan")
        if self.testplan_id:
            testplan = self.client.plans.get_specific_plane(
    ...
    def load_testrun(self):
        if self.testrun_id and self.testrun is None:
            self.testrun = self.client.runs.get_run(
                self.project_code,
                self.testrun_id,
                params=TestRunInclude.CASES,
            )

    def create_testrun(self, cases):
        if cases:
            res = self.client.runs.create(
            ...
            self.testrun_id = res.body.get("result").get("id")
            print(
    ...
    def start_step(self, identifier):
        ...
        if len(case_ids) > 1:
            ...
            raise MoreThenOneCaseIdException(
                "Test case decorated with several ids: {}".format(", ".join(case_ids))
            )
    ...
    def pytest_collection_modifyitems(self, session, config, items):
        ...
        with FileLock("qaseio.lock"):
            ...
            if no_ids:
                ...
                write_lines.append("This tests does not have test case ids:\n" + line)
        ...
        if item.nodeid in self.nodes_with_ids:
            ...
            for _id in self.nodes_with_ids[item.nodeid].get("ids", []):
                if _id not in self.missing_ids:
                    body = {"case_id": _id, "status": TestRunResultStatus.IN_PROGRESS}
                    result = self.client.results.create(
                        self.project_code, self.testrun_id, body=body
                    )
                    hashes.append(result.body.get("result").get("hash"))
    ...
    def finish_pytest_item(self, item):
        if item.nodeid in self.nodes_with_ids:
            ...
            for pos, values in results.get("steps", {}).items():
                steps.append({"position": pos, **values})
            attached = []
            if attachments:
            ...
            for hash in hashes:
                body = {
                    ...
                    "time_ms": int((time.time() - results.get("started_at")) * 1000),
                    "attachments": [attach.hash for attach in attached.body],
                    "steps": steps,
                }
                self.client.results.update(
                    self.project_code, self.testrun_id, hash, body=body
                )
            self.last_node = None
    ...
    def complete(self):
        if self.testrun_id and self.complete_run:
            ...
            res = self.client.runs.get_run(self.project_code, self.testrun_id)
            if res.body.get("status") == 1:
